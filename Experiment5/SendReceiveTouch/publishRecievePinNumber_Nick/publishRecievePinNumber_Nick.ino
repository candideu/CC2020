...
int katePinTouched;  ///the value Nick reads from Kates board via PN
int nickPinTouched; //the value I get locally
    ...
    while (!getMessage.finished()) 
    {
        ...
        if (msg.length() > 0) 
        {
          ...
          deserializeJson(inMessage, msg); // parse the  JSON value received
           ...
           katePinTouched = inMessage[JsonParamName2]; // the value of their Temperature sensor
